...
<Card class="py-2">
	{#if editor.document.composition && editor.document.layers.length > 0}
		...
		<CardContent class="px-2 space-y-1">
			{#each editor.document.layers as layer (layer.id)}
				<button
					class={cn(
						'w-full p-1 flex items-center gap-2 cursor-pointer hover:bg-accent/50 rounded-md border border-transparent',
						editor.document.isLayerActive(layer.id) && 'bg-accent/50 border-primary'
					)}
					onclick={(e) => {
						editor.document.activateLayer(layer.id, {
